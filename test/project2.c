...
void mostrarCarreirasComInicio(Carreira** c_root, Paragem** p_root) {
    char buffer[BUFFERSIZE], nome[BUFFERSIZE];
    char aspas = '\0';

    Carreira* curr_carreira;
    Paragem *p;
    ...
    p = encontraParagem(p_root, nome);

    if (p != NULL) {
        for (curr_carreira = *c_root; curr_carreira != NULL; curr_carreira = curr_carreira->prox) {
            if (encontraParagem(p_root, curr_carreira->origem.nome) == p) {
                printf("%s", curr_carreira->nome);
                if (curr_carreira->prox != NULL) { 
                    printf(" ");
                }
            }
        }
...
int main() {
	...
	while ((c = getchar()) != EOF) {
        ...
        case 'a': apagaTudo(&c_root, &p_root);
			break;
        case 's': mostrarCarreirasComInicio(&c_root, &p_root);
            break;
		}
	}
	return 0;
