...
int _numAeroportos = 0;	    /* numero de aeroportos introduzidos */
Aeroporto _aeroportos[MAX_NUM_AEROPORTOS];  /* vetor de aeroportos */

int _numVoos = 0;   /* numero de voos introduzidos */
Voo _voos[MAX_NUM_VOOS];    /* vetor de voos */

int _numReservas = 0;   /* numero de reservas introduzidas */
Reserva _reservas[60356];   /* vetor de reservas */

Data _hoje = { 1, 1, 2022 };    /* data atual do sistema */

const int _diasMesAc[] =    /* dias acumulados por mes (jan=1) */
...
void mostraReserva(int index) {
	printf("%s %d\n", _reservas[index].id,  _reservas[index].passageiros);
}
...
void adicionaReserva(){
	...
	if (leProximaPalavra(r.id) && leProximaPalavra(r.data)) {
		for (i = 0; i < _numReservas; i++)
			mostraReserva(i);
		return;
	} else {
		leProximaPalavra(r.cod);
		leProximaPalavra(r.passageiros);
		leAteFimDeLinha();
	}
